...
class EnergyFunction:
    ...
    def energyFunctionDerivative(self, inputs):
        ...
        for i in range(resolution):
            ...
            xdij1 = np.dot(self.normaliseVector(p5-p4), self.differentiateNormalVector(p3,p4,dp4,dp3)) + np.dot(self.normaliseVector(p3-p4), self.differentiateNormalVector(p5,p4,dp5,dp4))
            
            x2 = np.dot(self.normaliseVector(p4-p3), self.normaliseVector(p2-p3))
            xdij2 = np.dot(self.normaliseVector(p4-p3), self.differentiateNormalVector(p2,p3,dp2,dp3)) + np.dot(self.normaliseVector(p2-p3), self.differentiateNormalVector(p4,p3,dp4,dp3))
            
            x3 = np.dot(self.normaliseVector(p3-p2), self.normaliseVector(p1-p2))
            xdij3 = np.dot(self.normaliseVector(p3-p2), self.differentiateNormalVector(p1,p2,dp1,dp2)) + np.dot(self.normaliseVector(p1-p2), self.differentiateNormalVector(p3,p2,dp3,dp2))
            ...
            radialBendingDeriv = stiffness/length**3 * inputs[i]
            #All summed together to form dE/ddij
            #derivatives.append(lengthEnergyDeriv + dijDerivBend + radialBendingDeriv)
            #derivatives.append(lengthEnergyDeriv)
            derivatives.append(dijDerivBend)
            #derivatives.append(radialBendingDeriv)
        return derivatives
    ...
    def differentiateNorm(self, a, b, da, db):
        #returns the derivative of ||a-b||, as this is found a lot
        norm = self.norm(a-b)
        if norm == 0:
            return 0
        return np.dot(a-b, da-db)/norm
    ...
    def dxdij(self, x, xdij, p1, p2, p3, dp1, dp2, dp3, stiffness):
        ...
        #if x <= 1e-6:
        #    return 0
        return -stiffness*(2/(norm1 + norm2) * xdij/x**3 + (self.differentiateNorm(p3,p2,dp3,dp2) + self.differentiateNorm(p2,p1,dp2,dp1))/(norm1 + norm2)**2 *(1/x**2 - 1))
...
class CurveGenerator:
    ...
    def testEnergyDeriv(self, testInput, lengthPunish,stiffness):
        ...
        print("Energy: " , iterableEnergy.energyFunction(testInput))
        print("Derivative Approximation: " , -(iterableEnergy.energyFunction(testInput) - iterableEnergy.energyFunction(testInputMoved))/h) 
        print("Derivative: " , iterableEnergy.energyFunctionDerivative(testInput)[10])
        print("Derivatives: " , iterableEnergy.energyFunctionDerivative(testInput))
